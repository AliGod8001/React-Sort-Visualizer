...
import IndexUserArray from "./IndexUserArray";
...
const Index = () => {
  ...
  const onUserInsertValidArray = (array: number[]) => {
    actionClickHandler("reset-array", array);
  };

  const actionClickHandler = (action: string, userArray?: number[]) => {
    if (action === "start") {
      ...
      setStepsState({
        array: userArray ? userArray : randomArray(length, range),
        sortedArray: [],
        steps: [],
        colors: [],
      });
  ...
  return (
    <Wrapper>
      <IndexActions onActionClick={actionClickHandler} disabled={visualizing} />
      <IndexUserArray onUserInsertArray={onUserInsertValidArray} />
      <Setup
      ...
      {visualizing && stepState.steps.length ? (
        ...
        <BarsList
          array={stepState.array}
          colors={stepState.colors[0]}
          speed={speed}
        />
      )}
    </Wrapper>
